...
class ImageSender(NetworkAgent):
    ...
    def send_frame(self, frame: np.ndarray):
        ...
        _, jpg_frame = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), self._jpeq_quality])
...
class ImageReceiver(NetworkAgent):
    ...
    def __init__(self,
        ...
        self._receiver = imagezmq.ImageHub(open_port=f'tcp://{ip_addr}:{stream_port}', REQ_REP=False)
    ...
    def recv_frame(self) -> (float, np.ndarray):
        ...
        frame = cv2.imdecode(np.fromstring(jpg_frame, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
        return tstamp, frame
